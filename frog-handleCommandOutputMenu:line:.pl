...
if ($cmd =~ m/^lsblk\b/) {
    if ($line =~ m/^[\|\`]\-([a-z0-9]+)/) {
        system('frog-handleDriveMenuForDevice:.pl', '/dev/'.$1);
        exit 0;
    }
}

#freebsd
if ($cmd =~ m/^frog-generateDiskMenu/) {
    if ($line =~ m/^  ([a-z0-9]+)/) {
        system('frog-handleDiskMenuForDevice:.pl', '/dev/'.$1);
        exit 0;
    }
}
...
if ($cmd =~ m/^ifconfig\b/) {
    if ($line =~ m/^([a-z0-9]+):/) {
        system('frog-network-handleMenuForInterface:.pl', $1);
        exit 0;
    }
    $text = <<EOF;
Select a line that starts with the network interface name.
EOF
    system('frog', 'alert', $text);
    exit 0;
}
...
if ($cmd =~ m/^ps\b/) {
    if ($line =~ m/^[a-z][-a-z0-9]*\s+(\d+)/) {
        ...
        $text =~ s/\\/\\\\/g;
        $text =~ s/"/\\"/g;
        $cmd = sprintf('frog radio OK Cancel %s %s %s %s',
            qq{"$text"},
...
if ($cmd =~ m/^xset\b/) {
    if ($line =~ m/\bDPMS is Disabled\b/) {
        $text = "Enable DPMS?";
        $cmd = qq{frog confirm OK Cancel "$text"};
        $output = `$cmd`;
        chomp $output;
        if ($output eq 'OK') {
            system('xset', '+dpms');
...
if ($cmd =~ m/^xinput\b/) {
    ...
    if ($name =~ s/\bid=(\d+)\[slave.*//) {
        ...
        if ($output =~ m/\n\t*Device Enabled[ \(\)0-9]*:\s+([01])/) {
            $status = $1;
            if ($status) {
                $cmd = qq{frog confirm Disable Cancel "Disable $name (id=$id)?"};
                $output = `$cmd`;
                chomp $output;
                ...
                $cmd = qq{frog confirm Enable Cancel "Enable $name (id=$id)?"};
                $output = `$cmd`;
                chomp $output;
                if ($output eq 'Enable') {
...
if ($cmd =~ m/^iwconfig\b/) {
    if ($line =~ m/^([a-z0-9]+)/) {
        system('frog-wifi-selectNetwork.pl', $1);
        exit 0;
    }
    $text = <<EOF;
Select a line that starts with the network interface name.
