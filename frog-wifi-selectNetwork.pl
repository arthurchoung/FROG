...
$choice = `frog-wifi-generatePanel.pl | frog show WifiNetworksPanel`;
chomp $choice;
if (not $choice) {
exit 0;
}
...
if ($choice =~ m/\bencryption:on/) {
    ...
    if (-e $passwordFile) {
        @arr = ('frog', 'radio', 'OK', 'Cancel', qq{"What's the password?"});
    ...
    if ($password eq '') {
        $password = `frog input OK Cancel 'Enter wifi password:'`;
        chomp $password;
        if ($password ne '') {
    ...
    if ($password ne '') {
        system('frog-wifi-runWPASupplicantForInterface:essid:password:.pl', $interface, $essid, $password);
    }
} else {
