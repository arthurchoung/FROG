...
NSLog(@"Bad signature");
    ...
    if (signature[3] == 0) {
        ...
        if (signature[4] == 0) {
            if (signature[0] == 'v') {
                ...
                double (*func)(id, SEL, id) = imp;
                double val = func(target, sel, [args nth:0]);
                return nsfmt(@"%f", val);
            }
        } else if (signature[4] == '@') {
            if (signature[5] == 0) {
            ...
            if (signature[5] == 0) {
                ...
                if (signature[6] == 0) {
                    if (signature[0] == '@') {
                        ...
                        double (*func)(id, SEL, int, int, int) = imp;
                        double val = func(target, sel, [[args nth:0] intValue], [[args nth:1] intValue], [[args nth:2] intValue]);
                        return nsfmt(@"%f", val);
                    }
                } else if (signature[6] == 'i') {
                    if (signature[7] == 0) {
    ...
    } else if (signature[3] == 'q') {
        if (signature[4] == 0) {
            if (signature[0] == 'v') {
                void (*func)(id, SEL, long long) = imp;
                func(target, sel, [[args nth:0] longLongValue]);
                return target;
            }
        }
    }
