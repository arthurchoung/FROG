...
loop:
    # FIXME
    @mountlist = `frog-listBlockDevices.pl | frog-valueForKey:.pl mountpoint | sed '/^\$/d'`;
    chomp @mountlist;
    ...
    if (-e $mountpointsFile) {
        ...
        chomp @lines;
        if (scalar @lines) {
            @arr = ('frog', 'radio', 'OK', 'Cancel', qq{"Enter mount point for $device:"});
            $default = '1';
            $i = 1;
            foreach $line (@lines) {
                $line =~ s/([\"\\])/\\$1/g;
            ...
            if (not $default) {
                ...
                if ($result eq 'new') {
                    ...
                    if ($? != 0) {
                        system('frog', 'alert', "Unable to mount $device at $mountpoint");
                        exit 1;
                    }
                    chdir $mountpoint;
                    system('frog');
...
EOF
    ...
    $mountpoint = `frog input OK Cancel "$text" 'Mount point:'`;
    chomp $mountpoint;
    if ($? != 0) {
        exit 1;
    }
    ...
    $output = `frog-listBlockDevices.pl`;
    @lines = split "\n", $output;
    foreach $line (@lines) {
        if ($line =~ m/\bmountpoint:([^\s]+)/) {
            ...
            if ($mountpoint eq $str) {
                ...
                system('frog', 'alert', "$mountpoint is already in use");
                goto loop;
            }
        }
    }
