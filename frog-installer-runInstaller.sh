...
\n\
To set up the installation of FROG, click Continue.\
"\
  16 64 ; then
    exit 1
fi
...
if ! findmnt -P -M /mntinstaller ; then
    DEVICE=$( findfs LABEL=FROGINSTALLER )
    if [ $? -ne 0 ]; then
        # Unable to find FROGINSTALLER
...
RESULTS=( $( PATH="/root:$PATH" frog-installer-chooseDisk.pl ) )
if [ ${#RESULTS[@]} -eq 3 ]; then
    TEXT=$( PATH="/root:$PATH" frog-installer-installToDisk:bootPartition:systemPartition:.sh ${RESULTS[0]} ${RESULTS[1]} ${RESULTS[2]} )
else
