...
if fdisk -l "$DISKDEVICE" | grep '^Disklabel type: dos' >/dev/null ; then
    ...
    if ! PATH="/root:$PATH" dialog --yes-label "Install" --no-label "Cancel" --defaultno --yesno "Install FROG?\n\nDisk: $DISKDEVICE\nBoot Partition: $BOOTDEVICE\nSystem Partition: $SYSTEMDEVICE" 12 64 ; then
    ...
    PATH="/root:$PATH" frog-installer-installSystem.sh | PATH="/root:$PATH" dialog --backtitle "Linux Setup" --programbox 'INSTALLING SYSTEM' 8 80
    RESULT=${PIPESTATUS[0]}
    if [ $RESULT -eq 0 ]; then
        PATH="/root:$PATH" frog-installer-configureSystem.sh
        PATH="/root:$PATH" frog-installer-installMBRBootloaderToDisk:partition:UUID:.sh "$DISKDEVICE" "$BOOTDEVICE" "$UUID"
        SUCCESS=1
    fi
    umount /mntsystem
    sync
    ...
    if [ $RESULT -eq 0 ]; then
        PATH="/root:$PATH" frog-installer-configureSystem.sh
        PATH="/root:$PATH" frog-installer-installEFIBootloaderToPartition:UUID:.sh "$BOOTDEVICE" "$UUID"
        SUCCESS=1
    fi
    umount /mntsystem
    sync
