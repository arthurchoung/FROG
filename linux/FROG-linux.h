...
@interface NSObject(JRPmAWMBPLlCnrsfRXpdeKEgvQeKRDx)
...
+ (id)homeDir:(id)path;
+ (id)Wayland;
+ (id)openWindowForObject:(id)obj x:(int)x y:(int)y w:(int)w h:(int)h prev:(id)prev;
+ (id)waylandGetOutputs;
+ (void)waylandRotateMonitor:(int)index :(int)rotation;
+ (id)waylandMonitorForX:(int)x y:(int)y;
+ (id)waylandMonitorConfig;
...
+ (id)x11MonitorConfig;
@end
@interface NSConstantString(uAJwLZptvwtxWTpTTOUJUWeFcOhPwwR)
#ifndef BUILD_WITH_GNUSTEP_RUNTIME
#else
#endif
#ifdef BUILD_WITH_GNU_PRINTF
