...
@interface NSObject(fmeklwfmklsdmfklsdmklfskdfmksdf)
...
+ (void)runWindowManagerForObject:(id)object x:(int)x y:(int)y w:(int)w h:(int)h
{
    ...
    if (FROGNOFRAME) {
        dict = [_windowManager openWindowForObject:object x:x y:y w:w h:h overrideRedirect:NO propertyName:"FROGNOFRAME"];
        if (dict) {
            if ([object respondsToSelector:@selector(x11WindowMaskCString)]) {
                if ([object respondsToSelector:@selector(x11WindowMaskChar)]) {
                    ...
                    [_windowManager addMaskToWindow:win cString:cstr c:c];
                }
            }
        }
    } else {
...
NSLog(@"openButtonDownMenu obj %@ w %d h %d", obj, w, h);
...
w = 768.0 / 1.5;
h = 768;
id monitor = [Definitions x11MonitorForX:0 y:0];
...
//                    [Definitions drawToOpenGLTextureID:[_openGLTexture textureID] bytes:[bitmap pixelBytes] bitmapWidth:[bitmap bitmapWidth] bitmapHeight:[bitmap bitmapHeight] bitmapStride:[bitmap bitmapStride]];
    ...
    //            [bitmap fillRectangleAtX:0 y:0 w:w h:h];
                ...
                [bitmap fillRect:[Definitions rectWithX:0 y:0 w:w h:h]];
                [bitmap setColor:@"black"];
                id text = nsfmt(@"no openGL %@", [object description]);
                text = [bitmap fitBitmapString:text width:w-10];
                [bitmap drawBitmapText:text x:5 y:5];
            }
            GC gc = XCreateGC(_display, win, 0, 0);
...
NSLog(@"hotkey keyString %@", keyString);
        id hotKeyFiles = [[Definitions frogDir:@"Config/hotKeyFiles.csv"] parseCSVFile];
        for (int i=0; i<[hotKeyFiles count]; i++) {
            ...
            id menu = [[Definitions frogDir:path] parseCSVFile];
...
NSLog(@"handleX11MapRequest parent %x window %x", e->parent, e->window);
    ...
    BOOL noframe = [self doesWindow:e->window haveProperty:"FROGNOFRAME"];
...
NSLog(@"WM_NORMAL_HINTS %@", normalHints);
    ...
    if (attrs.x == 0) {
        if (attrs.y == 0) {
            ...
            id focusMonitor = [Definitions x11MonitorForX:focusX y:focusY];
            ...
            id cmd = nsarr();
            [cmd addObject:@"frog-getCoordinatesForNewWindow.pl"];
...
//        [dict setValue:name forKey:@"name"];
        [dict setValue:@"1" forKey:@"FROGNOFRAME"];
...
- (void)x11MaximizeTopHalf
{
    ...
    id monitor = [Definitions x11MonitorForX:oldX y:oldY];
    int monitorX = [monitor intValueForKey:@"x"];
...
- (void)x11AlignToMonitor
{
    ...
    int newX = monitorX;
    int newY = menuBarHeight + monitorY;
    [dict setValue:nsfmt(@"%d %d", newX, newY) forKey:@"moveWindow"];
}
- (void)x11MoveWindowDeltaX:(int)deltaX y:(int)deltaY
{
    id dict = self;
    int menuBarHeight = [_windowManager intValueForKey:@"menuBarHeight"];
    int newX = [dict intValueForKey:@"x"] + deltaX;
    int newY = [dict intValueForKey:@"y"] + deltaY;
    [dict setValue:nsfmt(@"%d %d", newX, newY) forKey:@"moveWindow"];
}
- (void)x11MoveWindowRelativeToMonitorX:(int)relativeX y:(int)relativeY
{
    id dict = self;
    int menuBarHeight = [_windowManager intValueForKey:@"menuBarHeight"];
    int oldX = [dict intValueForKey:@"x"];
    ...
    int monitorX = [monitor intValueForKey:@"x"];
    int monitorY = [monitor intValueForKey:@"y"];
    int newX = monitorX + relativeX;
    int newY = monitorY + relativeY;
    [dict setValue:nsfmt(@"%d %d", newX, newY) forKey:@"moveWindow"];
}
- (void)x11CenterWindow
{
    id dict = self;
    int menuBarHeight = [_windowManager intValueForKey:@"menuBarHeight"];
    int oldX = [dict intValueForKey:@"x"];
    ...
    int monitorWidth = [monitor intValueForKey:@"width"];
    int monitorHeight = [monitor intValueForKey:@"height"];
    int newX = monitorX + (monitorWidth-oldW)/2;
    int newY = menuBarHeight + (monitorHeight-menuBarHeight-oldH)/2;
    [dict setValue:nsfmt(@"%d %d", newX, newY) forKey:@"moveWindow"];
}
...
- (void)x11ToggleMaximizeWindow
{
    ...
    if ([dict valueForKey:@"revertMaximize"]) {
        ...
        id monitor = [Definitions x11MonitorForX:midX y:midY];
        int monitorX = [monitor intValueForKey:@"x"];
        int monitorWidth = rootWindowWidth;
        int monitorHeight = rootWindowHeight;
        if (monitor) {
...
- (void)x11MoveToMonitorDelta:(int)delta
{
    ...
    int midX = [dict intValueForKey:@"x"] + [dict intValueForKey:@"w"]/2;
    int midY = [dict intValueForKey:@"y"] + [dict intValueForKey:@"h"]/2;
    int monitorIndex = [Definitions x11MonitorIndexForX:midX y:midY];
