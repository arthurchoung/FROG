...
static int FROG_argc = 0;
static char **FROG_argv = 0;
...
NSLog(@"unable to set signal handler for SIGPIPE");
    ...
    extern void FROG_initialize_stdout(FILE *);
    extern void FROG_initialize(FILE *);
    FROG_initialize_stdout(stdout);
    FROG_initialize(stderr);
#else
    extern void FROG_initialize_stdout(FILE *);
    extern void FROG_initialize(FILE *);
    FROG_initialize_stdout(stdout);
    ...
    id pool = [[NSAutoreleasePool alloc] init];
        ...
        id frogDir = [Definitions frogDir];
...
NSLog(@"Unable to set PATH");
        ...
        if (setenv("SUDO_ASKPASS", [[Definitions frogDir:@"frog-getPassword.pl"] UTF8String], 1) != 0) {
...
{
    id cmd = nsarr();
    [cmd addObject:@"frog-about-text.pl"];
    ...
    if ([output isEqual:@"More Info..."]) {
        cmd = nsarr();
        [cmd addObject:@"frog-about-moreText.pl"];
        id text = [[cmd runCommandAndReturnOutput] asString];
        cmd = nsarr();
        [cmd addObject:@"frog"];
        [cmd addObject:@"alert"];
