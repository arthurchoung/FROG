...
@interface X11RootWindow : IvarObject
@end
@implementation X11RootWindow
...
{
    id path = [Definitions frogDir:@"Config/rootWindowMenu.csv"];
    [self showRootWindowMenu:event path:path];
}
- (void)handleRightMouseDown:(id)event
{
    id path = [Definitions frogDir:@"Config/rightButtonRootWindowMenu.csv"];
    [self showRootWindowMenu:event path:path];
}
- (void)showRootWindowMenu:(id)event path:(id)path
{
    ...
    id obj = [[path parseCSVFile] asX11Menu];
