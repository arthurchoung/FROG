...
+ (void)updateWindowPositionsForExpose:(id)windows monitor:(id)monitor
{
    ...
    id cmd = nsarr();
    [cmd addObject:@"frog-packRectanglesIntoWidth:height:..."];
    ...
    for (int i=0; i<[windows count]; i++) {
        ...
        if ([window intValueForKey:@"FROGNOFRAME"]) {
...
- (void)tileWindows
{
    ...
    for (int i=0; i<[objectWindows count]; i++) {
        ...
        int centerX = x + w/2;
        int centerY = y + h/2;
        int monitorIndex = [Definitions x11MonitorIndexForX:centerX y:centerY];
...
- (void)handleEnterWindowEvent:(id)event context:(id)x11dict
{
    ...
    if (![x11dict intValueForKey:@"FROGNOFRAME"]) {
